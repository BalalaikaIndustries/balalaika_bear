defmodule BalalaikaBear.UsersTest do
  ...
  test "gets nearby users" do
    use_cassette "users_get_nearby" do
      params = %{
        access_token:
          "27c4a2e59524fc677d41418bd59c046e6591ed8c2633ede8a0a6c04fbf792d6ba8468146a48b0a599bdfb",
        latitude: 30.313497,
        longitude: 59.938531
      }
  ...
  test "reports user" do
    use_cassette "users_report" do
      params = %{
        user_id: 13_886_715,
        access_token:
          "2cf34377fc5fac1beb5baa9a3346faf15653f99431164730b16a7797b184ade9925bca7c935457203b153",
        type: "porn"
      }
  ...
  test "searches for users" do
    use_cassette "users_search" do
      params = %{
        access_token:
          "31a1853977980cff82e354d1d6b642dbebe4402fe07ec2eaa0fd0ac8f3fe1156965364a3bc927b8400533",
        q: "Ayrat Badykov"
      }
